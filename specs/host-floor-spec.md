...
- Four of those five, **IQ4_NL, IQ4_XS, Q4_1 and Q5_1**, HAVE a host
...
Those are **three device predicates**. The host path is not one of the
four parts,
...
- **3.2** When `decodable(ty)` is read for block geometry
  ...
  escape hatch goes with it, because there is nothing left to escape.
...
- **4.1** When any type is runnable, `Linear.matvecInto` has an arm for
  it, asserted over every type. This is the check that does not exist
  today.
...
- **5.2** When MXFP4 reaches `matvecInto`, it dispatches to a host
  mat-vec written for it. It is the one format whose floor is genuinely
...
- **6.1** When a format has a decoder and a host mat-vec, it ships. Its
  device kernels land afterwards, each an optimization measured against
...
- The four MXFP4 device kernels with no row, and the wave-cooperative
  portability gap, are the route table's and the kernels'. They are
  named in §1.3 so this spec does not absorb them.
- A kernel-coverage check would have caught the MXFP4 stranding, meaning
  a check that every `*Launch*` in `QuantKernel` has a row or is
  declared bench-only. It belongs with the route table, not here.
