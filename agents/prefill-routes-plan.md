...
- [x] **7.5.1** Q4_1 and Q5_1 — affine per-32 legacy blocks, a hard refusal
      today. Block decoders are ~15 lines each.
      DONE 2026-09-14, as the FOUR-PART stack the scope note below
      demands, because a decoder alone is the trap it describes:

        1. decoders  `Quant.q41Block` / `q51Block`, w = d*q + m with the
                     quant UNBIASED. Bit-exact against ggml's own
                     `to_float` over ggml's own `ggml_quantize_chunk`
                     bytes, 64 blocks each — every bit of Q5_1's 32-bit
                     qh exercised.
        2. host      `q41MatVecIntoAt` / `q51MatVecIntoAt`. The affine
                     term factors out: d*sum(q*x) + m*sum(x), so the min
                     is one extra accumulator per block, not an add per
                     element. That rank-1 shape is what every device
                     route has to carry, and a decode-only test cannot
                     see it — hence a mat-vec test against
                     dequantize-then-multiply.
        3. bind gate `Linear.packedSupported`, opened ONLY with 1/2/4.
        4. device    `q41MatVecKernel` / `q51MatVecKernel` for decode
                     (the activation sum rides the eight vector loads the
                     dot already makes), and `q41F16CoopX1/X3` /
                     `q51F16CoopX1/X3` for prefill. Both ggml blocks are
                     ALREADY dword-aligned — 20 and 24 bytes — so unlike
                     Q4_0/Q5_0 neither needs a repack: `coopBlockWords`
                     5 and 6, `coopNeedsRepack` untouched.

      END-TO-END, on checkpoints llama-quantize produced from the 8B:

        q4_1  coop q4_1 4096 4096   prefill-mode batched 198
              prefill 1043 tok/s   decode 28.2 tok/s   RSS 5.96 GB
        q5_1  coop q5_1 4096 4096   prefill-mode batched 198
              prefill  717 tok/s   decode 22.4 tok/s   RSS 6.86 GB

      No `batch-refused` on either. That is the whole point of doing all
      four parts: the format does not merely LOAD, it routes batched like
      any other. (A first q5_1 decode read 4.03 tok/s on a file
      llama-quantize had just written — the cold-page-cache artifact the
      bench-first-model-runs-cold note predicts; warm and matched it is
      22.4, the expected ratio for 24-byte against 20-byte blocks.)

      FIXTURES: `q4_1.bin/.f32`, `q5_1.bin/.f32`, from ggml's reference
      quantizer over the same real Llama-3.1-8B token_embd values every
      other fixture in that directory uses. Generated by a throwaway C
      program under `tmp/` linking libggml-base — llama.cpp's own code as
      the oracle, and nothing non-cajeta lands in `tools/`.
- [~] **7.5.2** The IQ family (IQ1..IQ4, TQ1_0, TQ2_0) — codebook types,
      genuinely a separate spec. No checkpoint on this box uses one.
      THE FAMILY SPLITS, and the split is the finding. "Codebook types"
      is true of IQ1/IQ2/IQ3 and FALSE of IQ4: IQ4_NL and IQ4_XS are
      NON-LINEAR, not codebook — the nibble indexes a SIXTEEN-entry
      value table, which is 16 bytes, not the kilobyte grids IQ2/IQ3
      carry. IQ4_NL is Q4_0's block byte for byte; IQ4_XS is a 256
      super-block with a six-bit per-32 scale. Both were therefore in
      reach of the existing machinery and are DONE 2026-09-14, four
      parts each:

        decoders   `Quant.iq4nlBlock` / `iq4xsBlock`, bit-exact against
                   ggml's own to_float over its own quantizer.
        host       `iq4nlMatVecIntoAt` / `iq4xsMatVecIntoAt`.
        bind gate  `Linear.packedSupported`.
        device     `iq4nlMatVecKernel` / `iq4xsMatVecKernel` for decode —
                   the table decodes through ONE Vector.lut4 byte-permute,
                   the same trick that made the MXFP4 kernel beat
                   llama.cpp — and `iq4nl/iq4xsF16CoopX1+X3` for prefill.
                   IQ4_NL repacks 18 bytes to 5 dwords like Q4_0; IQ4_XS
                   is 136 bytes = 34 dwords and needs none.

      END-TO-END on checkpoints llama-quantize produced from the 8B:

        iq4_nl  coop iq4_nl 4096 4096  batched 198  prefill 598  decode 32.9
        iq4_xs  coop iq4_xs 4096 4096  batched 198  prefill 541  decode 35.1

      TWO DEFECTS THE RUN SURFACED, both fixed here:
      (a) `GgufFile.typeName` knew none of these ids, so every route and
      refusal printed `type_20` / `type_23`. Added, with tq1_0/tq2_0.
      (b) The coop refusal blamed the COLUMNS for a type that simply has
      no coop kernel — IQ4_XS was refused for "cols not a multiple of
      the tile" at inDim 4096, which is a fine multiple of 256. A reason
      that names the wrong cause is worse than no reason: it sends the
      reader to the shape. Split into "no coop GEMM kernel for this
      format".

      STILL OPEN, and deliberately left refusing: TQ1_0/TQ2_0 (ternary,
      BitNet-only) and the true codebook tier IQ1_S/IQ1_M/IQ2_XXS/
      IQ2_XS/IQ2_S/IQ3_XXS/IQ3_S. Fixtures for tq1_0 and tq2_0 are
      generated and committed, and their decoders are ~20 lines each —
      but a decoder alone is the trap 7.5.1's scope note describes, so
      they stay OUT of `Quant.supported()` until they get all four
      parts. The codebook tier needs its own spec: the grids are
